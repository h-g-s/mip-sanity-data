...
def solve_spp_external(n_elements, n_sets, values, members, timeout=60):
    """Solve the SPP MPS with mipster to get a certified optimal value;
    returns (obj, status) where status is "optimal", "timelimit",
    "infeasible", or an error string."""
    import re
    import subprocess
    import tempfile
    ...
    with tempfile.NamedTemporaryFile(suffix=".mps", delete=False) as tmp:
        mps_path = tmp.name
    write_spp_mps(mps_path, n_elements, n_sets, values, members)
    ...
    try:
        result = subprocess.run(
            [mipster_bin, mps_path, "-sec", str(timeout), "-solve"],
            capture_output=True, text=True, timeout=timeout + 30
        )
    except subprocess.TimeoutExpired:
        Path(mps_path).unlink(missing_ok=True)
        return None, "timelimit"

    Path(mps_path).unlink(missing_ok=True)
    out = result.stdout

    obj_m = re.search(r"Objective value:\s*([\-0-9.e]+)", out)
    obj_val = float(obj_m.group(1)) if obj_m else None
...
SPECS = [
    ...
    # NOTE: value_type is "integer" for all baseline instances to reproduce
    # the originally committed fixtures byte-for-byte (verified against
    # test/fixtures/spp_*.mps.gz in the mipster repo). The generator supports
    # other value_types (continuous/large/fractional) for the search harness.
    (10,  15,  42,  "random",     3, "integer",     10),
    (12,  18,  42,  "k_partite",  3, "integer",     15),
    (15,  20,  137, "geometric",  3, "integer",     20),
    (20,  25,  42,  "scheduling", 4, "integer",     20),

    # Medium instances
    (30,  50,  42,  "random",     4, "integer",     60),
    (35,  60,  137, "k_partite",  4, "integer",     60),
    (40,  70,  42,  "geometric",  5, "integer",     90),
...
def main():
    ...
    # Repo layout: generators/spp/gen_spp_fixtures.py -> repo_root/mips
    ...
    for n_elem, n_sets, seed, pattern, k, value_type, timeout in SPECS:
        ...
        if status == "optimal":
            ...
            # Compress directly into repo mips/
