...
def solve_gap_external(n_agents, n_tasks, costs, weights, capacity, timeout=60):
    """Solve the GAP MPS with mipster to get a certified optimal value;
    ...
    with tempfile.NamedTemporaryFile(suffix=".mps", delete=False) as tmp:
        mps_path = tmp.name
    write_gap_mps(mps_path, n_agents, n_tasks, costs, weights, capacity)
    ...
    obj_val = round(float(obj_m.group(1))) if obj_m else None
...
# (n_agents, n_tasks, seed, alpha, cost_type, weight_type, timeout)
#
...
def main():
    # Repo layout: generators/gap/gen_gap_fixtures.py -> repo_root/mips
    ...
    for (n_ag, n_tk, seed, alpha, cost_type, weight_type, timeout) in SPECS:
        ...
        obj, status = solve_gap_external(n_ag, n_tk, costs, weights, capacity, timeout)
