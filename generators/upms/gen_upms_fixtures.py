...
import gzip, random, re, subprocess, sys
...
def verify_with_solver(mps_path, timeout=300):
    """Solve with mipster; return (is_optimal, obj_value) or (False, None)."""
    try:
        r = subprocess.run(
            [MIPSTER, str(mps_path), "-sec", str(timeout), "-solve"],
            capture_output=True, text=True, timeout=timeout + 30
        )
        out = r.stdout + r.stderr
        is_opt = "Optimal solution found" in out
        obj = None
        m = re.search(r"Objective value:\s*([\-0-9.eE]+)", out)
        if m:
            try:
                obj = float(m.group(1))
            except ValueError:
...
def main():
    fixture_dir = Path(__file__).parent.parent.parent / "mips"
