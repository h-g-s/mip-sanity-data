...
def certify_optimum(mps_path, time_limit=600):
    """Solve to optimality with the mipster solver, return (obj, status)."""
    import re

    mipster_bin = Path.home() / "prog" / "mipster" / "bin" / "mipster"
    cert = str(mipster_bin) if mipster_bin.exists() else shutil.which("mipster")
    if cert is None:
    ...
    try:
        ...
        is_infeasible = "infeasible" in out.lower() and "Problem" in out
