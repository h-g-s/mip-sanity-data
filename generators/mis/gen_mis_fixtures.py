...
def solve_mis_external(n_vertices, weights, edges, timeout=60):
    """Solve the MPS with mipster to get a certified optimal value;
    returns (obj, status) where status is "optimal", "timelimit", or
    an error string."""
    import re
    import subprocess
    import tempfile
    ...
    with tempfile.NamedTemporaryFile(suffix=".mps", delete=False) as tmp:
        mps_path = tmp.name
    write_mis_mps(mps_path, n_vertices, weights, edges)
...
def main():
    # Repo layout: generators/mis/gen_mis_fixtures.py -> repo_root/mips
