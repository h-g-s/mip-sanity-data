...
def verify_with_external_solver(mps_path, timeout=300):
    """Solve with the mipster solver and return (objective value, status)."""
    mipster_bin = Path.home() / "prog" / "mipster" / "bin" / "mipster"
    try:
        result = subprocess.run(
            [str(mipster_bin), str(mps_path), "-sec", str(timeout), "-solve"],
            capture_output=True, text=True, timeout=timeout + 60
        )
        out = result.stdout + result.stderr

        obj_m = re.search(r"Objective value:\s*([\-0-9.eE]+)", out)
        is_optimal = "Optimal solution found" in out
...
def main():
    ...
    for name, graph_gen, n_terminals, seed, desc in STEINER_INSTANCES:
        ...
        obj, status = verify_with_external_solver(mps_path, timeout=120)
        print(f"  Solved: obj={obj}, status={status}")
    ...
    print("\nOptimal values:")
    for name, obj, status in results:
        if obj is None:
            print(f"  {name}: FAILED ({status})")
        else:
            print(f"  {name}: {obj:.2f} ({status})")
