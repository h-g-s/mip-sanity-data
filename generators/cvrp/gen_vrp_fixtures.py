...
# Repo layout: generators/cvrp/gen_vrp_fixtures.py -> repo_root/mips
MIPS_DIR = Path(__file__).resolve().parents[2] / "mips"
...
def write_cvrp_mps(name, K, customers, Q, demands, costs, desc):
    ...
    with gzip.open(MIPS_DIR / f"{name}.mps.gz", "wt") as f:
        f.write(f"NAME          {name}\n")
        f.write("ROWS\n")
        f.write(" N  OBJ\n")
