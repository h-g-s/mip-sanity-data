...
def solve_bpc_external(mps_path, n_items, capacity, weights, conflicts, timeout=120):
    """Solve bin packing with conflicts using the mipster solver (subprocess)."""
    import re
    import subprocess
    ...
    mipster_bin = Path.home() / "prog" / "mipster" / "bin" / "mipster"

    try:
        proc = subprocess.run(
            [str(mipster_bin), mps_path, "-sec", str(timeout), "-solve"],
            capture_output=True, text=True, timeout=timeout + 60,
        )
        out = proc.stdout + proc.stderr
    except Exception as exc:
        print(f"WARNING: external solver invocation failed: {exc}", file=sys.stderr)
        return None, None, n_bins

    obj_match = re.search(r"Objective value:\s*([\-0-9.eE]+)", out)
    optimal = "Optimal solution found" in out or "proven optimal" in out.lower()

    if obj_match:
        obj_val = float(obj_match.group(1))
        status = "optimal" if optimal else "timelimit"
        return obj_val, status, n_bins

    return None, "no_solution", n_bins
...
def main():
    ...
    scratch_dir = Path("/tmp/bpc_scratch")
    ...
    for n_items, capacity, seed, conf_pat, conf_dens, weight_pat, timeout in SPECS:
        ...
        # Write MPS
        mps_path = str(scratch_dir / f"{name}.mps")
        write_bpc_mps(mps_path, n_items, capacity, weights, conflicts)

        # Solve with external solver
        obj, status, n_bins = solve_bpc_external(mps_path, n_items, capacity, weights, conflicts, timeout)
